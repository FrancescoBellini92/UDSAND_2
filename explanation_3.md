...
3) We create a char-prefix map (for encoding the string) and a prefix_char map (to decode the encoded string) via a depth first traversal on the huffman tree,
...
Regarding space complexity, given n (number of character as our input), we have two data structures involved:
1) a linked list, whose space complexity is O(n)
2) a binary tree, whose space complexity also grows linearly with n
3) two hash maps, whose space complexity alsow grows linearly with n
